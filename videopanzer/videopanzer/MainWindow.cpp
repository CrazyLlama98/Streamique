    ...
    NAM(new QNetworkAccessManager(this))
{
    ...
    //connect(NAM, &QNetworkAccessManager::readyRead, this, &);
...
void MainWindow::onFinished(QNetworkReply *reply) {
    ...
    if(token.isNull()) {
        ...
        QNetworkRequest reqLobbies(QUrl("http://streamique.azurewebsites.net/api/Lobbies/"));
        ...
        if(replyData.isArray()) {
            ...
            for(auto it : lobbyArray){
                auto userNickname = it.toObject()["name"].toString();
...
void MainWindow::handleCall() {
    ...
    peermanager::currentLobby = usersList->currentItem()->text();
